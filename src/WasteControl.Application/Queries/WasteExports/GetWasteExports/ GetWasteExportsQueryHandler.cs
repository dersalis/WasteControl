...
{
    public class GetWasteExportsQueryHandler : IRequestHandler<GetWasteExportsQuery, IEnumerable<WasteExportDto>>
    {
        ...
        public async Task<IEnumerable<WasteExportDto>> Handle(GetWasteExportsQuery request, CancellationToken cancellationToken)
        {
            ...
            {
                Id = w.Id,
                ReceivingCompanyName = w.ReceivingCompany is not null ? $"[{w.ReceivingCompany?.Code}] {w.ReceivingCompany?.Name}" : "",
                TransportCompanyName = w.TransportCompany is not null ? $"[{w.TransportCompany?.Code}] {w.TransportCompany?.Name}" : "",
